...
PARAM_BOUNDS: dict[str, tuple[float, float]] = {
    ...
    # 렌즈 왜곡(대각선 절반으로 정규화한 규약).
    #
    # 아래쪽은 모델이 접히는 한계에서 온다. k2=k3=0 일 때 g'(1) = 1 + 3*k1 이
    # 0 이하가 되는 k1 <= -1/3 부터 이미지 모서리에서 상이 접혀 역함수가
    # 아예 존재하지 않는다(distortion.is_invertible 참고). k1, k2, k3 가
    # 섞이면 한계가 달라지므로 정확한 차단은 solver 의 장벽이 맡고, 여기서는
    # 명백히 벗어난 값만 걸러 낸다.
    #
    # 위쪽은 실제 렌즈 기준이다. 이 규약에서 광각 드론 카메라도 |k1| 가
    # 0.2 를 넘는 일이 드물다. 예전 값(±2.0)은 물리적 의미가 없을 뿐 아니라
    # 해가 그쪽으로 도망가도 눈치채기 어려웠다.
    "k1": (-0.40, 0.60),
    "k2": (-0.40, 0.40),
    "k3": (-0.20, 0.20),
    # 접선 왜곡은 보통 1e-3 규모다.
